...
def encode_and_save(args):
    ...
    elif args.model == 'rff-hdc':
        print("Encoding to binary HDC with random fourier features.")
        encoder = RandomFourierEncoder(
            input_dim=input_dim, gamma=args.gamma, binary=True, output_dim=args.dim)
    elif args.model == 'rff-gvsa':
        print("Encoding to cyclic group VSA with random fourier features.")
        encoder = RandomFourierEncoder(
            input_dim=input_dim, gamma=args.gamma, binary=False, gorder=args.gorder, output_dim=args.dim)
    else:
...
def load(args):
    ...
    return mem, train_hd, y_train, test_hd, y_test
...
def prepare_data(args):
    mem, train_hd, y_train, test_hd, y_test = load(args)
